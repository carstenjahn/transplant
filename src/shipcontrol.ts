...
export class ShipAndCamera {
    ...
    private shieldsRemainingFrames = 0;
    private scoreLives;

    constructor(scoreLives: ScoreLives) {
    ...
    public shipCollided() {
        if(this.shieldsRemainingFrames === 0) {
            this.ship.material = new THREE.MeshBasicMaterial({color: 0xffff00});
            this.shieldsRemainingFrames = 100;
            this.scoreLives.substractLife();
        }
    }
    ...
    public applyShipControl(shipControl: ShipControl) {
        ...
        if (this.shieldsRemainingFrames > 0) {
            this.shieldsRemainingFrames -= 1;
            if (this.shieldsRemainingFrames === 0) {
                this.ship.material = new THREE.MeshBasicMaterial({color: 0x0000ff});
            }
        }
    }
}